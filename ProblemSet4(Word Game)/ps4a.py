...
def getWordScore(word, n):
    ...
    sum_letter_values=0
    for letter in word:
        sum_letter_values += SCRABBLE_LETTER_VALUES[letter]
    score = sum_letter_values * len(word)
    if (n==len(word)):
        score=score+50
    return score
...
def playHand(hand, wordList, n):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    while(sum(hand.values()) > 0):
        ...
        c_word=raw_input('Enter word, or a "." to indicate that you are finished: ')
        if c_word==".":
            print 'Goodbye! Total score: ',total,'points.'
            break
        elif isValidWord(c_word, hand, wordList)==False:
